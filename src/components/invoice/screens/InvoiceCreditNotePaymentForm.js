...
export default function InvoiceCreditNotePaymentForm({ invoiceId, onSuccess }) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm({
    defaultValues: { payment_type: 'credit_note' },
  })
  ...
  async function loadCreditNotes() {
    const api = await get('/v1/admin/template/credit_notes')
  ...
  return (
    <Form onSubmit={handleSubmit(onSubmit)} className="credit-note-payment-form">
      <input type="hidden" {...register('payment_type')} value="credit_note" />
      ...
      <Form.Group className="mb-2">
        ...
        <Form.Select
          size="sm"
          isInvalid={!!errors.credit_note}
          {...register('credit_note', { required: 'Credit note is required' })}
        >
          <option value="">Select credit note</option>
          {creditNotes.map((note) => (
            <option key={note.id} value={note.id}>
              {note.name || `Credit note #${note.id}`}
            </option>
          ))}
        </Form.Select>
        <Form.Control.Feedback type="invalid">{errors.credit_note?.message}</Form.Control.Feedback>
      </Form.Group>
...
InvoiceCreditNotePaymentForm.propTypes = {
