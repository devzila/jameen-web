...
export default function EditSecurityStaff({ after_submit, id }) {
  ...
  async function fetchSecurityStaff() {
    ...
    if (response.ok && securityStaffData?.data) {
      const staff = securityStaffData.data
      setValue('name', staff.name)
      setValue('email', staff.email)
      setValue('mobile_number', staff.mobile_number)
      setValue('active', staff.active)
      setValue(
        'property_ids',
        (staff.properties || []).map((element) => ({
          value: element.id,
          label: element.name,
        })),
      )
  ...
  async function onSubmit(data) {
    ...
    await put(`/v1/admin/security_staffs/${id}`, { security_staff: processed_data })
    if (response.ok) {
      setVisible(false)
      after_submit()
      toast.success('Security staff updated successfully')
    } else {
      setErrors(response.data?.errors || {})
      toast.error(response.data?.message || 'Unable to update security staff')
    }
  }
  ...
  const avatarSrc = imageView || staffData.avatar || DEFAULT_AVATAR
            ...
            <div className="d-flex align-items-center" style={{ gap: '14px' }}>
              ...
              <div className="d-flex flex-column">
                ...
                <span style={{ fontSize: '12px', fontWeight: 400, opacity: 0.9 }}>
                  {staffData.name || 'Update staff details'}
                </span>
              </div>
            </div>
          </Modal.Title>
