...
export default function EditMaintenanceStaff({ after_submit, id }) {
  ...
  async function onSubmit(data) {
    ...
    await put(`/v1/admin/maintenance_staffs/${id}`, { maintenance_staff: processed_data })
    if (response.ok) {
      setVisible(false)
      after_submit()
      ...
      toast.error(response.data?.message || 'Unable to update maintenance staff')
    }
  }
