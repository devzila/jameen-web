...
export default function AddMaintenanceStaff({ after_submit }) {
  ...
  async function onSubmit(data) {
    ...
    await post(`/v1/admin/maintenance_staffs`, { maintenance_staff: processed_data })
    if (response.ok) {
      setVisible(false)
      after_submit()
      reset()
      ...
      toast.error(response.data?.message || 'Unable to add maintenance staff')
    }
  }
