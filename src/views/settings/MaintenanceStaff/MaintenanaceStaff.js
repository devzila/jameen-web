...
export default function MaintenanceStaff() {
  ...
  async function fetchMaintenanceStaff() {
    ...
    if (response.ok && api?.data) {
      ...
      toast.error('Unable to load maintenance staff')
    }
    setLoading(false)
  }
  ...
  return (
    <div style={{ padding: '20px' }}>
      <style>{`
        .maintenance-staff-table tbody tr { transition: background-color .15s ease; }
        .maintenance-staff-table tbody tr:hover { background-color: #f5fdfe; }
      `}</style>
      ...
      >
        ...
        >
          <div className="d-flex align-items-center" style={{ gap: '12px' }}>
            ...
            >
              <CIcon icon={freeSet.cilSettings} size="lg" />
            </div>
            <div>
              ...
              <small style={{ color: '#8a94a6' }}>
                {searchKeyword ? filteredMaintenanceData.length : maintenanceData.length} total
              </small>
            </div>
          </div>
        ...
        <div className="table-responsive">
          <table
            className="table maintenance-staff-table mb-0"
          ...
          >
            ...
            <tbody>
              {filteredMaintenanceData.map((staff) => (
                <tr key={staff.id}>
                  ...
                  <td style={bodyCellStyle}>
                    <div className="d-flex justify-content-center">
                      <Dropdown>
                        ...
                        >
                          <EditMaintenanceStaff id={staff.id} after_submit={reload_callback} />
                        </Dropdown.Menu>
                      </Dropdown>
                    </div>
                  </td>
              ...
              {!loading && filteredMaintenanceData.length === 0 && (
                <tr>
                  <td
                    colSpan={5}
