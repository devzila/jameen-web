...
export default function Allotment() {
  ...
  async function fetchAllotment() {
    try {
      const allotmentData = await get(`/v1/admin/premises/properties/${propertyId}/Allotment`)
      console.log(allotmentData)
      if (allotmentData && allotmentData.data) {
        setAllotment(allotmentData.data)
      }
    } catch (error) {
        ...
        <CCol md="4">
          <CCard className=" p-5  m-3 border-0  " style={{ backgroundColor: '#00bfcc' }}>
            <div className="d-flex align-items-center justify-content-center h-100 ">
              <img className="rounded-circle w-50 " src={allotment.avatar || logo} />
            </div>
          </CCard>
        </CCol>
        <CCol md="8">
          <CCard className=" p-3 my-3 me-3" style={{ border: '0px' }}>
            ...
            <CRow className="">
              <CCol className="p-2 mt-0 fw-light theme_color">
                Name
                <CCardText className="fw-normal text-black">{allotment?.name || '-'}</CCardText>
              </CCol>
              <CCol className="p-3 mt-0 fw-light theme_color">
                City
                <CCardText className="fw-normal text-black">{allotment?.city || '-'}</CCardText>
              </CCol>
              <CCol className="p-3 mt-0 fw-light theme_color">
                Use Type
                <CCardText className="fw-normal text-black">{allotment?.use_type || '-'}</CCardText>
              </CCol>
              <CCol className="p-3 mt-0 fw-light theme_color">
                Payment Term
                <CCardText className="fw-normal text-black">
                  {allotment?.payment_term || '-'}
                </CCardText>
              </CCol>
            </CRow>
            <CRow>
              <CCol className="p-3 mt-0 fw-light theme_color">
                Overdue Days
                <CCardText className="fw-normal text-black">
                  {allotment?.invoice_overdue_days || '-'}
                </CCardText>
              </CCol>
              <CCol className="p-3 mt-0 fw-light theme_color">
                Invoice No Prefix
                <CCardText className="fw-normal text-black">
                  {allotment?.invoice_no_prefix || '-'}
                </CCardText>
              </CCol>
              <CCol className="p-3 mt-0 fw-light theme_color">
                Invoice Day
                <CCardText className="fw-normal text-black">
                  {allotment?.invoice_day || '-'}
                </CCardText>
              </CCol>
              <CCol className="p-3 mt-0 fw-light theme_color">
                Created At
