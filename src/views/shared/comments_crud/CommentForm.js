...
export default function CommentForm({ refresh_callback, id, close_edit_callback, api }) {
  ...
  async function onSubmit(data) {
    if (id) {
      const endpoint = await put(api, {
        comment: data,
      })
      close_edit_callback()
    } else {
      const endpoint = await post(api, {
        comment: data,
      })
    }
    if (response.ok) {
